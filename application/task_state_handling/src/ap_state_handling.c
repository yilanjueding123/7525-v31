...
void ap_state_handling_power_off(void)
{
	...
	static INT8U p_cnt=0;
		static INT8U usb_connect=0;
		static INT8U usb_cnt=0;
		INT8U type;
		INT8U  i=0;
	...
	if(sys_pwr_key1_read())
			...
			if(usb_cnt ==0)
				{
				watchdog_ctrl(4,0,1);				//0.125s
				watchdog_init();
				DBG_PRINT("open_watchdog\r\n");
				while (1);
				//看门狗迅速复位
				}
			#endif

...
{
#if 0
	OSQPost(DisplayTaskQ,
		 (void *) (MSG_DISPLAY_TASK_ICON_CLEAR | (ICON_MD_STS_0 | (ICON_MD_STS_1 << 8) | (ICON_MD_STS_2 << 16))));
	OSQPost(DisplayTaskQ,
		 (void *) (MSG_DISPLAY_TASK_ICON_CLEAR | (ICON_MD_STS_3 | (ICON_MD_STS_4 << 8) | (ICON_MD_STS_5 << 16))));
...
INT8S ap_video_record_md_active(INT8U * md_tick, INT32U state)
{
	*md_tick			= 0;
	...
	if ((ap_state_config_record_time_get() == 0) &&
		 (vfsFreeSpace(MINI_DVR_STORAGE_TYPE) >> 20) < CARD_FULL_SIZE_RECORD)
	{
	...
	if (! (video_record_sts & VIDEO_RECORD_BUSY))
	{
		...
		else 
		{
			if (motion_detect_timerid == 0xFF)
			{
				...
				sys_set_timer((void *) msgQSend, (void *) ApQ, MSG_APQ_MOTION_DETECT_TICK, motion_detect_timerid,
					 MOTION_DETECT_CHECK_TIME_INTERVAL);
			}
		}
	}
...
INT8S ap_video_record_func_key_active(INT32U event)
{
	...
	if (video_record_sts & VIDEO_RECORD_BUSY)
	{
		...
		if ((ap_video_record_sts_get() &VIDEO_RECORD_MD) == 0)
	...
	else 
	{
		if ((video_record_sts & VIDEO_RECORD_UNMOUNT) == 0)
		{
			...
			if (video_record_sts == 0 || video_record_sts == VIDEO_RECORD_MD)
			{
				...
				if (ap_video_record_sts_get() &VIDEO_RECORD_MD)
				{
					if ((event == MSG_APQ_FUNCTION_KEY_ACTIVE) || (event == MSG_APQ_MODE) ||
						 (event == MSG_APQ_MENU_KEY_ACTIVE))
					{
						ap_video_record_md_icon_clear_all();
						ap_video_record_md_disable();
						ap_state_config_md_set(0);
				...
				if (disk_free_size < CARD_FULL_SIZE_RECORD)
				{
					...
					{
						msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_FREE_FILESIZE_CHECK, NULL, NULL,
							 MSG_PRI_NORMAL);
					}
				...
				if (ap_state_config_md_get())
				{
					...
					{
						msgQSend(PeripheralTaskQ, MSG_PERIPHERAL_TASK_MOTION_DETECT_START, NULL, NULL,
							 MSG_PRI_NORMAL);
						ap_video_record_sts_set(VIDEO_RECORD_MD);
					}
					...
					{
						motion_detect_timerid = VIDEO_PREVIEW_TIMER_ID;
						sys_set_timer((void *) msgQSend, (void *) ApQ, MSG_APQ_MOTION_DETECT_TICK,
							 motion_detect_timerid, MOTION_DETECT_CHECK_TIME_INTERVAL);
					}
				}
			}
			else 
...
void ap_video_record_cycle_reply_action(void)
{
	...
	if (next_file_info.file_handle != -1)
	{
		...
		else 
		{
			...
			next_ptr			= (INT8U *)
			next_file_info.file_path_addr;
			curr_ptr			= (INT8U *)
			curr_file_info.file_path_addr;
		...
		if ((total_size <= 1024) && ((ap_state_config_record_time_get() == 4) ||
			 (ap_state_config_record_time_get() == 5)))
		{
			size				= 300;
		}
		else 
...
void ap_video_record_reply_action(STOR_SERV_FILEINFO * file_info_ptr)
{
	...
	{
		bkground_del_disable(0);					//0 // back ground auto delete enable
	...
	if (src.type_ID.FileHandle >= 0)
	{
		...
		else 
		{
			...
			if (cyclic_record_timerid == 0xFF)
			{
				...
				sys_set_timer((void *) msgQSend, (void *) ApQ, MSG_APQ_CYCLIC_VIDEO_RECORD, cyclic_record_timerid,
					 time_interval);
			}
...
#if C_AUTO_DEL_FILE 						== CUSTOM_ON
			{
				...
				if (curr_file_info.storage_free_size < bkground_del_thread_size_get())
				{
					...
					if (bkground_del_disable_status_get() == 0)
					{
						...
						temp				= 0xFFFFFFFF;
						msgQSend(StorageServiceQ, MSG_STORAGE_SERVICE_VIDEO_FILE_DEL, &temp, sizeof(INT32U),
							 MSG_PRI_NORMAL);
					}
				}
			}
...
void video_calculate_left_recording_time_enable(void)
{
	...
	if (freespace > (CARD_FULL_SIZE_RECORD << 20))
		freespace -= (CARD_FULL_SIZE_RECORD << 20);
	else 
		freespace = 0;

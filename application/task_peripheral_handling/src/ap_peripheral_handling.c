...
static INT8U	led_record_to_motion_flag = 1;
...
void ap_peripheral_key_init(void)
{
	...
	for (i = 0; i < USE_IOKEY_NO; i++)
	{
		...
		if (1)//key_map[i].key_io)		//MOTION_KEY:IO_A0腔硉峈0
		{
			key_map[i].key_cnt	= 0;
...
void ap_peripheral_key_register(INT8U type)
{
	...
	if (type == GENERAL_KEY)
	{
		...
		key_map[4].key_io	= VIDEO_KEY;
		key_map[4].key_function = (KEYFUNC)ap_peripheral_functionb_key_exe;
		key_map[4].key_active = FUN_KEYD_ACTIVE;
		key_map[4].key_long = 0;

		key_map[5].key_io	= MOTION_KEY;
		key_map[5].key_function = (KEYFUNC)ap_peripheral_functionc_key_exe;
		key_map[5].key_active = 0;
		key_map[5].key_long = 0;
...
void ap_peripheral_functionb_key_exe(INT16U * tick_cnt_ptr)
{
	...
	if (!s_usbd_pin)
	{
		...
		{			
			if(video_record_sts & 0x04)		//痄雄淈聆
			{
				if((video_record_sts & 0x02))
				{
					led_record_to_motion_flag = 0;
					msgQSend(ApQ, MSG_APQ_VIDEO_RECORD_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
				}
				else
				{
